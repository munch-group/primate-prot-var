...
class GnomADTSVProcessor:
    ...
    def download_constraint_file(self, version='v2.1.1'):
        ...
        urls = {
            ...
            'v4.0': 'https://storage.googleapis.com/gcp-public-data--gnomad/release/4.0/constraint/gnomad.v4.0.constraint_metrics.tsv'
        }
    ...
    def download_lof_by_gene(self):
        ...
        """
        url = 'https://storage.googleapis.com/gcp-public-data--gnomad/papers/2019-flagship-lof/v1.0/gnomad.v2.1.1.lof_by_gene.txt.bgz'
...
class GnomADBatchAPI:
    ...
    def batch_query_genes(self, gene_list: List[str], dataset: str = "gnomad_r3"):
        ...
        """
        query = """
        query BatchGenes($geneSymbol: String!, $datasetId: DatasetId!) {
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'errors' in data:
                            if 'Gene not found' not in str(data['errors']):
                                logger.warning(f"API returned errors for {gene_symbol}: {data['errors']}")
                        ...
                        if 'data' in data and 'gene' in data['data']:
                            ...
                            if gene_data and 'variants' in gene_data:
                                ...
                                if variant_count > 0:
                                    logger.info(f"Found {variant_count} LoF variants for {gene_data.get('symbol', gene_symbol)}")
                    ...
                    if attempt < self.max_retries - 1:
                        import time
                        logger.warning(f"Timeout for {gene_symbol}, retrying ({attempt + 1}/{self.max_retries})...")
                        time.sleep(2)
                    else:
                        logger.warning(f"Timeout for {gene_symbol} after {self.max_retries} retries, skipping")
                        failed_genes.append(gene_symbol)

                except Exception as e:
                    logger.warning(f"Error querying gene {gene_symbol}: {e}")
                    failed_genes.append(gene_symbol)
                    break

            if (i + 1) % 100 == 0:
                logger.info(f"Processed {i + 1}/{len(gene_list)} genes, {len(all_variants)} LoF variants found")

        if failed_genes:
            logger.warning(f"Failed to query {len(failed_genes)} genes due to errors")
...
class GnomADParquetProcessor:
    ...
    def download_parquet_files(self):
        ...
        base_url = "https://storage.googleapis.com/gcp-public-data--gnomad/release/3.1.2/ht/"
...
class GnomADPipeline:
    ...
    def run(self) -> pd.DataFrame:
        ...
        if self.method == 'api':
            ...
            # Map version to dataset ID
            dataset_map = {
                'v2.1.1': 'gnomad_r2_1',
                'v3.1.2': 'gnomad_r3',
                'v4.0': 'gnomad_r4'
            }
            dataset = dataset_map.get(self.version, 'gnomad_r2_1')
            df = processor.batch_query_genes(genes, dataset=dataset)
